# lumi-notifications user information

This module provides some tools to send messages to some popular tools.
...
These tools can be an alternative for the lack of mail support in Slurm. E.g., you can use the
tool to send a message at the start of the job. Sending a message at the end is of course also
possible, but depending on how the job script is written, it may not be always be reached if the
job is killed for some reason.

The recommended way of working with both tools is to use a configuration file that contains at
least the necessary credentials so that they are not visible in, e.g., the process list.
The default configuration file on LUMI is `~/.LUMI-notifications.ini`. It is recommended to use
file permissions 600 (though as your home directory already has 700 permissions on LUMI, it
doesn't really matter on LUMI).

## pushover

### Getting credentials and using the configuration file

The credentials consist of both a user key and an API token. To obtain those credentials:
